...
export function meta({ params }: Route.LoaderArgs) {
  return [
    { title: `Vecno Transaction ${params.transactionId} | Vecnoscan` },
    {
      name: "description",
      content: "Explore Vecno transaction. View sender, recipient, amount, status, and associated blocks and more.",
    },
    {
      name: "keywords",
      content: "Vecno transaction, transaction ID, blockchain transfer, sender, receiver, transaction status",
    },
  ];
}
...
export default function TransactionDetails({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <>
      <div className="mt-22 flex w-full flex-col rounded-4xl bg-white p-4 text-left text-black sm:p-8">
        ...
        <div className="grid grid-cols-1 gap-x-14 gap-y-2 sm:grid-cols-[auto_1fr]">
          ...
          />
          <FieldName name="To" infoText="The (output) address(es) where the VE in this transaction were sent to." />
          <FieldValue
            value={
              <ul>
                {transaction.outputs ? (
      ...
      <div className="flex w-full flex-col gap-x-18 gap-y-6 rounded-4xl bg-white p-4 text-left text-black sm:p-8">
        ...
        {isTabActive("general") && transaction && (
          <div className="grid w-full grid-cols-1 gap-x-18 gap-y-2 rounded-4xl bg-white text-left text-nowrap text-black sm:grid-cols-[auto_1fr]">
            ...
            <FieldValue value={<VeLink copy to={transaction.transaction_id} linkType="transaction" />} />
            <FieldName
              name="Subnetwork ID"
            ...
            <FieldValue
              value={
                transaction.accepting_block_hash ? (
                  <VeLink link linkType="block" to={transaction.accepting_block_hash} />
                ) : (
