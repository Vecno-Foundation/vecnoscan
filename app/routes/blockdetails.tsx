...
export default function Blocks({ loaderData }: Route.ComponentProps) {
  ...
  const getAddressFromOutpoint = (txId: string, outpointIndex: number) => {
      ...
      if (tx.transaction_id === txId) {
        for (const output of tx.outputs || []) {
          if (output.index === outpointIndex) {
            return <VeLink linkType={"address"} to={output.script_public_key_address} shorten link mono />;
          }
        }
      }
    }
    return (
      <>
        <VeLink linkType={"transaction"} to={txId} shorten link mono />
        {` #${outpointIndex}`}
      </>
    );
  };
        ...
        <PageTable
          ...
          rows={
            block?.transactions.map((transaction) => [
              <VeLink linkType="transaction" to={transaction.verboseData.transactionId} link shorten mono />,
              <ul>
